...
Reports
Reference: Enums
ClientType · OpportunityStage · OpportunitySource · ProposalStatus · PricingRequestStatus · ContractStatus · RenewalStatus · ActivityType · ActivityStatus · Priority · DocumentCategory · RelatedToType
Typical Workflows
Response Codes
Authentication
POST /api/auth/login
...
Users
List and retrieve users that belong to the current tenant. Useful for populating assignment dropdowns (opportunity owner, activity assignee, pricing request assignee) and for user management views.

GET /api/users
List all users in your organisation with optional filters.
...
role	string	Filter by role name — Admin, SalesManager, BusinessDevelopmentManager, SalesRep
searchTerm	string	Searches firstName, lastName, email
isActive	bool	Filter active (true) or inactive (false) users
Response 200
...

{
  "items": [
    {
      "id": "4446e829-4761-4da5-8f38-8c20a778500c",
      "email": "alice@acme.com",
      "firstName": "Alice",
      "lastName": "Smith",
...
GET /api/users?role=SalesRep
GET /api/users?role=Admin
GET /api/users?role=SalesManager
GET /api/users?role=BusinessDevelopmentManager
GET /api/users?searchTerm=alice&isActive=true
GET /api/users/{id}
Get a single user by ID. Returns 404 if the user does not belong to the current tenant.
...
lossReason is required when moving to stage 6 (Closed Lost).
...
assignedToId is optional. If provided, status is auto-set to InProgress. The requesting user is taken from the token.
...
Both fields are optional. renewalOpportunityId links this renewal to an existing opportunity if one was created to track the renewal deal.
...
relatedToType	int	Yes	Entity type to attach to — see RelatedToType
...
GET /api/users?isActive=true                  → all active users (for any assignment field)
GET /api/users?role=SalesRep&isActive=true    → sales reps only (opportunity owner, activity assignee)
GET /api/users?role=SalesManager&isActive=true → managers only (pricing request assignee)
Full sales cycle

1.  POST /api/auth/login
2.  GET  /api/users?isActive=true             → load users for assignment dropdowns
